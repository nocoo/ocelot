...
状态：**移动导航与最近更新已实现并通过本地验收；正式发行及生产证据见 Release** · 2026-09-12
...
这些是本轮按用户授权选定的实现方案；完成情况及证据见下方验证记录。
...
- Worker 按源 commit 每批处理 100 个候选，私有 R2 保存进度和最终 50 篇；客户端
  在全部候选完成后显示结果。首页预览前 5 篇，右上角及预览“查看全部”打开完整列表。
  列表独立加载、取消、失败及重试；刷新列表期间保留 README 区域高度和阅读位置。
- Model 单测验证 UTC 更新时间排序、UTF-16 路径 tie-break、50 篇上限、少于 50、
  空列表、大小写扩展名、隐藏文件、根目录工程说明与 2 MiB 边界。
- 真实 D1/R2 集成验证超过 1,000 篇的分批扫描、最后一批出现最新文章、断点重试、
  完整缓存命中、Worker 实例更换、缓存淘汰重建，以及读取缓存前授权。
- 验证重命名、删除、同时间、相同 tree 不同 commit 的回退历史、条件同步及清理；
  `0002_recent_revisions.sql` 在已有快照和复用 tree 的当前提交上完成回填，重跑
  migrations 不产生重复版本，不丢失旧快照。测试不依赖真实 vault。
- GraphQL 验证固定 commit、转义路径、只读 query、UTC 规范化、仓库/commit 身份、
  缺失历史、限流、权限撤销及不完整响应；不返回 GitHub 原始错误中的私有信息。
- 浏览器验证首页预览与完整 50 篇的顺序、实际文章导航、删除/重命名更新、重载缓存、
  不足 50 篇、加载/失败/重试/空状态，以及 320px 布局。桌面、手机的最近更新截图
  已人工查看，相关 axe 检查均无违规。

最终本地结果：**213 项单元/Worker 集成测试、40 项 Playwright 流程全部通过**。
覆盖率（含未执行的一方非 View runtime 源文件）：statements **100%（1205/1205）**、
branches **98.56%（1028/1043）**、functions **100%（223/223）**、
lines **100%（1071/1071）**。类型、Biome、前端生产构建、Worker dry run、类型生成
无漂移、Actionlint 1.7.12、OSV Scanner 2.5.1 与仓库历史 Gitleaks 8.30.1 通过。
浏览器 HTML 报告、手势数值附件和合成截图由 CI browser job 保存；不把真实私有文章
的路径、内容或截图加入公开仓库与发行附件。

### 发行证据的归属

版本由根 `package.json` 经 `scripts/release.ts` 从 `0.2.0` 升到 `0.3.0`；
发布脚本只在对应 push CI 和独立生产部署成功后创建不可变 tag/Release。
目标生产地址为 [ocelot.hexly.ai](https://ocelot.hexly.ai)。最终远端 SHA、CI/CD、
Release URL 及实际已认证 HTTP/mobile smoke 结果收录在
[v0.3.0 Release](https://github.com/nocoo/ocelot/releases/tag/v0.3.0)，以其中实际完成的
记录为准；本地结果不代替线上验收。完成本次验收后不继续监控。
