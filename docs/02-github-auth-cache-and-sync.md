...
状态：**架构提案，待讨论；尚未实现或配置凭据**

更新：2026-09-11

## 建议结论

针对“单用户、私有仓库主要属于一个 GitHub 用户”的已确认场景，首版建议：

**Cloudflare Access + 细粒度只读 PAT + Worker + D1 元数据 + 私有 R2 内容缓存。**

以 Git commit/tree/blob SHA 表示版本，按需读取文件。浏览器可见时定期检查
活动仓库的分支 HEAD，只有版本变化才更新目录快照。这些是建议，不能视作
用户已接受的认证与存储决策。
...
| D1 | 已添加仓库、稳定 repo ID、分支、当前/待发布 commit、tree SHA、ETag、检查时间、同步状态 | 小规模结构化状态，支持原子条件更新 |
...
这是在 Cloudflare 账户内保存私人内容的副本。建议首版使用可重建缓存，
按写入时间设置有限保留期；移除仓库先撤销应用内可读状态，再清理其对象前缀。
具体保留天数在实现前确定。
...
提议首版默认：**打开仓库、页面恢复可见、手动刷新时检查；阅读过程中每
...
- 展示“最后成功检查时间”和“源仓库提交时间”两个不同概念。检查失败、
  GitHub 限流、凭据失效也使用不同状态；按 `Retry-After` 与 rate-limit
...
建议为私有仓库设置短授权复查窗口，与成功的 HEAD 检查共用，首版拟为
60 秒。仅有 Access 登录不能把一次 GitHub 授权延长为永久缓存访问权。
...
对于当前 Vite + React 应用，建议首版由 Worker 负责认证、数据访问和缓存，
...
| 决策 | 当前推荐 | 状态 |
| --- | --- | --- |
| 私有仓库认证 | 细粒度 PAT；若希望自动令牌续期则选 GitHub App | 待讨论 |
| 私有内容保存 | D1 元数据 + 私有 R2，按需缓存 | 待讨论 |
| 阅读渲染位置 | 浏览器渲染，Worker 提供受保护数据 | 待讨论 |
| 更新触发 | 可见时 60 秒条件检查 + 打开/手动检查 | 待讨论 |
| 授权复查 | 短窗口，过期且无法验证时停止服务端私有缓存读取 | 待讨论 |
| 缓存保留期 | 有限期、可重建；移除仓库时清理 | 具体天数待定 |
