...
| 私有内容保存 | D1 元数据 + 私有 R2，按需缓存 | 本轮采用 |
| 阅读渲染位置 | 浏览器渲染，Worker 提供受保护数据 | 本轮采用 |
| 更新触发 | 可见时 60 秒条件检查 + 打开/手动检查 | 本轮采用 |
| 授权复查 | 短窗口，过期且无法验证时停止服务端私有缓存读取 | 本轮采用 |
| 缓存保留期 | 对象保留 30 天，可重建；移除仓库时清理 | 本轮采用 |
